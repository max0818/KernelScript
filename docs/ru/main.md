[Назад](readme.md)

# KernelScript

Здесь собрана вся документация

## Главное

- [Значения](documentation/values.md)
- [Типы](documentation/types.md)
- [Особенности](documentation/special.md)

## Документация

- 📃 [Комментарии](documentation/syntax/0_comments.md)
- 📃 [Операторы](documentation/syntax/1_operators.md)
- 📦 [Переменные](documentation/syntax/2_variables.md)
- 📙 [Массивы](documentation/syntax/3_arrays.md)
- 📚 [Объекты](documentation/syntax/4_objects.md)
- ❔ [Условные операторы](documentation/syntax/5_ifelse.md)
- 🔄️ [Циклы](documentation/syntax/6_loops.md)
- ⚙️ [Функции](documentation/syntax/7_functions.md)
- 🏛️ [Классы](documentation/syntax/8_classes.md)
- 🚩 [Флаги](documentation/syntax/9_flags.md)
